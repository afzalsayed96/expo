...
class UpdatesDevLauncherController : UpdatesInterface {
  ...
  companion object {
    private var sInstance: UpdatesDevLauncherController? = null
    val instance: UpdatesDevLauncherController
      get() {
        return checkNotNull(sInstance) { "UpdatesDevLauncherController.getInstance() was called before the module was initialized" }
      }

    fun initialize(context: Context): UpdatesDevLauncherController {
      if (sInstance == null) {
        sInstance = UpdatesDevLauncherController()
      }
