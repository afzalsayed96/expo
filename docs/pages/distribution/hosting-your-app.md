...
In order to configure your standalone binary to pull OTA updates from your server, you’ll need to define the URL where your `index.json` file is hosted. When using EAS Build, just set the [`updates.url` property in app.json](/versions/latest/config/app/#url) to point to that url.
