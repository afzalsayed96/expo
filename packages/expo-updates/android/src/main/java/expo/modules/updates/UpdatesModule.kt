...
class UpdatesModule(
  ...
  override fun getConstants(): Map<String, Any> {
    ...
    try {
      val updatesService: UpdatesInterface = updatesService
      constants["isEmergencyLaunch"] = updatesService.isEmergencyLaunch
      constants["isMissingRuntimeVersion"] = updatesService.configuration.isMissingRuntimeVersion
      val launchedUpdate = updatesService.launchedUpdate
      if (launchedUpdate != null) {
        constants["updateId"] = launchedUpdate.id.toString()
        constants["manifestString"] = if (launchedUpdate.manifest != null) launchedUpdate.manifest.toString() else "{}"
      }
      val localAssetFiles = updatesService.localAssetFiles
      ...
      constants["isEnabled"] = updatesService.configuration.isEnabled
      constants["releaseChannel"] = updatesService.configuration.releaseChannel
      constants["isUsingEmbeddedAssets"] = updatesService.isUsingEmbeddedAssets
    } catch (e: Exception) {
  ...
  fun reload(promise: Promise) {
    try {
      ...
      updatesService.relaunchReactApplication(object : LauncherCallback {
  ...
  fun checkForUpdateAsync(promise: Promise) {
    try {
      val updatesService = updatesService
      if (!updatesService.configuration.isEnabled) {
        promise.reject(
          "ERR_UPDATES_DISABLED",
      ...
      val databaseHolder = updatesService.databaseHolder
      val extraHeaders = ManifestMetadata.getServerDefinedHeaders(
        databaseHolder.database, updatesService.configuration
      )
      databaseHolder.releaseDatabase()
      updatesService.fileDownloader.downloadManifest(
        updatesService.configuration,
        extraHeaders,
        context,
        object : ManifestDownloadCallback {
          ...
          override fun onSuccess(updateManifest: UpdateManifest) {
            ...
            if (updatesService.selectionPolicy.shouldLoadNewUpdate(
